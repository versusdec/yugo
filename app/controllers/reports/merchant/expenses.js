define(['microcore', 'apexcharts', 'json!/js/ru', 'mst!/reports/merchant/expenses', "popup"],
  function (mc, ApexCharts, ruLang, view, popup) {
      ...
      function draw($scope) {
          const options = {
              series: [{
                  name: mc.i18n('table.total'),
                  type: 'line',
                  data: [30, 25, 36, 30, 45, 35, 64, 52, 59, 36, 39]
              }, {
                  name: mc.i18n('reports.expenses.title'),
                  type: 'column',
                  data: [23, 11, 22, 27, 13, 22, 37, 21, 44, 22, 30]
              }, {
                  name: mc.i18n('table.returns'),
                  type: 'column',
          ...
          chart = new ApexCharts($($scope).find('.expenses_chart')[0], options);
          chart.render();
      }

      mc.events.on('reports.merchant.stats.expenses.toggle.series', ({name, btn}) => {
          btn.classList.toggle('toggled')
          chart.toggleSeries(name);
      });

      mc.events.on("reports.merchant.stats.expenses.sort", async (e) => {
          if (e.length == 2) {
