define(['microcore', 'apexcharts', 'json!/js/ru'], function (mc, ApexCharts, ruLang) {
    ...
    mc.events.on('dashboard.merchant.stats.leads.toggle.series', (name) => {
        chart.toggleSeries(name);
    });

    mc.events.on("dashboard.merchant.stats.leads.sort", async (e) => {
        if (e.length == 2) {
    ...
    mc.events.on('dashboard.merchant.leads.list', async $scope => {
        let hash = mc.router.hash();
        let data = await mc.api.call('stats.dashboard.merchant.leads',
    ...
    return async function ($scope, $params) {
        ...
        mc.events.push('dashboard.merchant.leads.list')
        // draw($scope);
    }
});